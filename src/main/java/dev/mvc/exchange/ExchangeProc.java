...
//    exchangeVO.setPasswd(passwd_encoded);
  ...
  public ArrayList<ExchangeVO> list_all() {
    ArrayList<ExchangeVO> list = this.exchangeDAO.list_all();
    return list;
  }

  @Override
  public ArrayList<ExchangeVO> list_by_classifyno(int classifyno) {
    ArrayList<ExchangeVO> list = this.exchangeDAO.list_by_classifyno(classifyno);
    return list;
  }
  ...
  public ExchangeVO read(int exchangeno) {
    ExchangeVO exchangeVO = this.exchangeDAO.read(exchangeno);
    return exchangeVO;
  }
  ...
  public int youtube(HashMap<String, Object> map) {
    int cnt = this.exchangeDAO.youtube(map);
    return cnt;
  }

  @Override
  public ArrayList<ExchangeVO> list_by_classifyno_search(HashMap<String, Object> hashMap) {
    ArrayList<ExchangeVO> list = this.exchangeDAO.list_by_classifyno_search(hashMap);
    return list;
  }

  @Override
  public int list_by_classifyno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.exchangeDAO.list_by_classifyno_search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<ExchangeVO> list_by_classifyno_search_paging(HashMap<String, Object> map) {
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Exchange.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Exchange.RECORD_PER_PAGE;
    /*
    ...
    ArrayList<ExchangeVO> list = this.exchangeDAO.list_by_classifyno_search_paging(map);
...
//  
//  <DIV id='paging'>
//    <span class='span_box_1'><A href='list_by_classifyno.do?word=&now_page=1&classifyno=4'>1</A></span>
//  <span class='span_box_2'>2</span>
//  <span class='span_box_1'><A href='list_by_classifyno.do?word=&now_page=3&classifyno=4'>3</A></span>
//    </DIV>
//  </DIV> 
//  <!-- 페이지 목록 출력 부분 종료 -->
   ...
   */
  @Override
  public String pagingBox(int classifyno, int now_page, String word, String list_file, int search_count, int record_per_page,
      int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?classifyno="+classifyno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?classifyno="+classifyno+"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int password_check(HashMap<String, Object> map) {
    ...
    int cnt = this.exchangeDAO.password_check(map);
    return cnt;
  }

  @Override
  public int update_text(ExchangeVO exchangeVO) {
    int cnt = this.exchangeDAO.update_text(exchangeVO);
    return cnt;
  }

  @Override
  public int update_file(ExchangeVO exchangeVO) {
    int cnt = this.exchangeDAO.update_file(exchangeVO);
    return cnt;
  }

  @Override
  public int delete(int exchangeno) {
    int cnt = this.exchangeDAO.delete(exchangeno);
    return cnt;
  }

  @Override
  public int count_by_classifyno(int classifyno) {
    int cnt = this.exchangeDAO.count_by_classifyno(classifyno);
    return cnt;
  }

  @Override
  public int delete_by_classifyno(int classifyno) {
    int cnt = this.exchangeDAO.delete_by_classifyno(classifyno);
    return cnt;
  }

  @Override
  public int count_by_memberno(int memberno) {
    int cnt = this.exchangeDAO.count_by_memberno(memberno);
    return cnt;
  }

  @Override
  public int delete_by_memberno(int memberno) {
    int cnt = this.exchangeDAO.delete_by_memberno(memberno);
    return cnt;
  }

  @Override
  public int increaseReplycnt(int exchangeno) {
    int count = exchangeDAO.increaseReplycnt(exchangeno);
    return count;
  }
